...
function STERILISEDATA(){
  ...
  //var sheets=['Orders','Mixing','Production','Printing','Labelling','Packaging','Shipping','References/ProductCodes'];
  //var sheets=[ 'References/ProductCodes']
  ...
  for(var i=0;i<sheets.length;i++){
    try{
      ...
      for(var j=0;j<list.length;j++){
        ...
        }else if(list[j].final_status == 'started' || list[j].lineItemMove == 'Busy'){
        ...
        if(list[j].runtime == 0 || !list[j].runtime){
          data[list[j].batch].starttime= 0;
        }else{
          data[list[j].batch].starttime= list[j].runtime;
        }
